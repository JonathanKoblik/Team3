...
intl_cc_px=pd.DataFrame(intl_cc_df["AOK"]["close"])

intl_cm_px=pd.DataFrame(intl_cm_df["AOM"]["close"])
intl_cg_px=pd.DataFrame(intl_cg_df["AOR"]["close"])
intl_ca_px=pd.DataFrame(intl_ca_df["AOA"]["close"])

# These steps were needed to remove an abviously wrong outlier when the ITOT price "jumped" in one day from under 50 to above 100:
# ---------------------------------------------------
intl_cs_px_unfiltered=pd.DataFrame(intl_cs_df["ITOT"]["close"])
z_scores=stats.zscore(intl_cs_px_unfiltered)
# ---------------------------------------------------
intl_cs_px=intl_cs_px_unfiltered[(np.abs(stats.zscore(intl_cs_px_unfiltered))<3).all(axis=1)]
...
cc_d_r=intl_cc_px.pct_change().dropna()
cc_c_r=(1+cc_d_r).cumprod()
cc_h_g=cc_c_r*initial_investment


intl_cc_hist_c_return=(1+intl_cc_px.pct_change()).dropna().cumprod()*initial_investment
intl_cm_hist_c_return=(1+intl_cm_px.pct_change()).dropna().cumprod()*initial_investment
intl_cg_hist_c_return=(1+intl_cg_px.pct_change()).dropna().cumprod()*initial_investment
intl_ca_hist_c_return=(1+intl_ca_px.pct_change()).dropna().cumprod()*initial_investment
intl_cs_hist_c_return=(1+intl_cs_px.pct_change()).dropna().cumprod()*initial_investment
...
if risk_tolerance == 1:
    ...
    st.write('This portfolio invests in iShares AOM ETF and maintains 40% allocation to Stocks and 60% allocation to Bonds.')
    st.write('________________________________________________________________')
    ...
    st.write('AOM PORTFOLIO ASSET ALLOCATION:')
    st.bar_chart(aom_ac)
    st.write('AOM PORTFOLIO ASSET ALLOCATION BY COUNTRY:')
    st.bar_chart(aom_gm_weights)
    
    st.write('GROWTH OF HYPOTHETICAL $10,000')
    st.line_chart(intl_cm_hist_c_return)
    ...
    col1.subheader("Asset Allocation")
    col1.bar_chart(aom_ac)
    col2.subheader("Country Asset Allocation")
    col2.bar_chart(aom_gm_weights)
    
    col1, col2 = st.beta_columns([1, 1])
    col1.subheader("Growth of Hypothetical $10k")
    col1.line_chart(intl_cm_hist_c_return)
    col2.subheader("Monte Carlo Simulated 1-y Return")
    col2.line_chart(intl_cm_simulated_MEAN_returns_df)
    ...
    st.write('AOR PORTFOLIO ASSET ALLOCATION:')
    st.bar_chart(aor_ac)
    st.write('AOR PORTFOLIO ASSET ALLOCATION BY COUNTRY:')
    st.bar_chart(aor_gm_weights)
    
    st.write('GROWTH OF HYPOTHETICAL $10,000')
    st.line_chart(intl_cg_hist_c_return) 
    ...
    st.write('AOK PORTFOLIO ASSET ALLOCATIONn:')
    st.bar_chart(aok_ac)
    st.write('AOK PORTFOLIO ASSET ALLOCATION BY COUNTRY:')
    st.bar_chart(aok_gm_weights)
    
    st.write('GROWTH OF HYPOTHETICAL $10,000')
    st.line_chart(intl_cc_hist_c_return) 
    ...
    st.write('AOA PORTFOLIO ASSET ALLOCATION:')
    st.bar_chart(aoa_ac)
    st.write('AOA PORTFOLIO ASSET ALLOCATION BY COUNTRY::')
    st.bar_chart(aoa_gm_weights)
    
    st.write('GROWTH OF HYPOTHETICAL $10,000')
    st.line_chart(intl_ca_hist_c_return) 
    ...
    st.write('This portfolio invests in iShares ITOT ETF and maintains 100% allocation to US Stocks.')
    st.write('________________________________________________________________')
    ...
    st.write('ITOT PORTFOLIO ASSET ALLOCATION:')
    st.bar_chart(itot_ac)
    st.write('ITOT PORTFOLIO ASSET ALLOCATION BY COUNTRY:')
    st.bar_chart(itot_gm_weights)
    
    st.write('GROWTH OF HYPOTHETICAL $10,000')
    st.line_chart(intl_cs_hist_c_return)
